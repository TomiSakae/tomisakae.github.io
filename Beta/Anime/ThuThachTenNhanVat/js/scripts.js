            ...
            if (charactersData.data && charactersData.data.length > 0) {
                ...
                if (mainCharacters.length > 0) {
                    ...
                    mainCharacters.forEach(character => {
                        ds_nhan_vat_chinh.push(character);
...
async function TaoCauHoi() {
    ...
    $("#anh_cau_hoi").attr('src', ds_nhan_vat_chinh[nhan_vat_duoc_chon].character.images.jpg.image_url);
    ...
    $("#dap_an" + dap_an_dung).text(ds_nhan_vat_chinh[nhan_vat_duoc_chon].character.name);

    // Tạo một danh sách ngẫu nhiên của các đáp án (bao gồm cả đáp án đúng)
    let danh_sach_dap_an_ngau_nhien = [];
    for (let i = 0; i < so_luong_nhan_vat; i++) {
        if (ds_nhan_vat[i].character.name != ds_nhan_vat_chinh[nhan_vat_duoc_chon].character.name) {
            danh_sach_dap_an_ngau_nhien.push(ds_nhan_vat[i].character.name);
        }
    }
    ...
    for (let i = 1; i <= 4; i++) {
        if (i != dap_an_dung && danh_sach_dap_an_ngau_nhien.length != 0) {
            $("#dap_an" + i).removeClass("d-none");
            $("#dap_an" + i).text(danh_sach_dap_an_ngau_nhien.pop());
        }
        if (danh_sach_dap_an_ngau_nhien.length == 0) {
            $("#dap_an" + i).addClass("d-none");
        }
    }
...
function KetQua() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        <div id="ket_qua`+ (i + 1) + `" class="` + kt_dung_sai + ` khung-ket-qua d-flex align-items-center">
                    <img src="`+ ds_nhan_vat_anime[i].character.images.jpg.image_url + `"
    ...
    $('[id^="ket_qua"]').click(function () {
        ...
        let searchTerm = ds_anime_cau_hoi[Number(so_phia_sau) - 1].attributes.canonicalTitle + " character";
